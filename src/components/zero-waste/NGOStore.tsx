...
export function NGOStore() {
  const bulkStoreItems = useAppStore((s) => s.bulkStoreItems) || [];
      ...
      <div className="flex flex-col gap-6 pt-6 relative z-10 pb-24">
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            <Store size={28} className="text-[#1B5E8A]" />
            <h1 className="text-h1">NGO Store</h1>
          </div>
          <p className="text-body text-text-secondary mt-1">Bulk food at discounted prices for pickup only</p>
        </motion.div>
        ...
        >
          <AnimatePresence mode="popLayout">
            {bulkStoreItems.length > 0 ? (
              bulkStoreItems.map((item) => (
                <motion.div key={item.id} variants={fadeInUp} layoutId={`store-${item.id}`}>
                  <LightCard className="p-0 overflow-hidden flex flex-col group cursor-pointer h-full border border-[#E8E8E4]">
                    <div className="relative h-28 w-full overflow-hidden bg-[#FAFAF8]">
                      ...
                      <div className="absolute top-2 left-2 bg-[#3A7D52] text-white text-[10px] font-bold px-2 py-0.5 rounded-md shadow-sm uppercase tracking-wide">
                        Special Price
                      </div>
                    </div>
              ...
              <p className="text-center text-gray-500 mt-8 col-span-2">No bulk items available right now.</p>
            )}
          </AnimatePresence>
        </motion.div>
      </div>
