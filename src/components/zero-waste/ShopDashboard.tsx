...
export function ShopDashboard() {
  ...
  const wastePrevented = todaySold * 0.4; // kg approx
    ...
    <div className="flex h-full flex-col bg-zw-bg-base">
      ...
      <div className="sticky top-0 z-30 border-b border-zw-border/60 bg-white px-5 pb-3 pt-4">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-zw-accent-500 to-zw-accent-700">
        ...
        {/* Tabs */}
        <div className="mt-3 flex gap-1 rounded-xl bg-zw-bg-muted p-1">
          {[
            ...
            <button
              ...
              className={`relative flex-1 rounded-lg px-3 py-1.5 text-[12px] font-semibold transition-all ${
                tab === t.id
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                <StatCard
                  ...
                  trend="+3"
                  gradient="from-zw-primary-50 to-white"
              ...
              >
                ...
                <p className="mt-2 text-[12px] leading-relaxed text-zw-text-secondary">
                  Based on your sales pattern, <b>3 milk packs</b> and <b>2 vegetable packs</b> are
                  likely to remain unsold tomorrow. Pre-list them at discount now.
                </p>
                <button className="mt-3 rounded-lg bg-zw-primary-700 px-3 py-1.5 text-[11px] font-semibold text-white">
                  Pre-list with AI suggestion
                </button>
              </motion.div>

              {/* Quick action */}
              <button className="mt-4 flex w-full items-center justify-between rounded-2xl border border-zw-border bg-white p-4 text-left">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-zw-primary-50 text-zw-primary-700">
                    <Plus size={18} />
                  </div>
                  <div>
              ...
              {/* Expiring soon */}
              <h3 className="mt-5 mb-2 font-display text-[13px] font-bold uppercase tracking-wide text-zw-text-secondary">
                Expiring Soon
              </h3>
              <div className="space-y-2">
                {shopListings
                  ...
                  .map((l) => (
                    <div
                      key={l.id}
                      className="flex items-center gap-3 rounded-xl border border-zw-accent-200 bg-zw-accent-50 p-3"
                    >
                      <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-zw-accent-600 text-white">
                        <AlertCircle size={16} />
                      </div>
                      ...
                      <button className="rounded-lg bg-zw-accent-600 px-3 py-1.5 text-[11px] font-bold text-white">
                        Boost Discount
                      </button>
                    </div>
                  ))}
            ...
            >
              {activeOrders.map((order, i) => (
                <motion.div
                  ...
                  className="overflow-hidden rounded-2xl border border-zw-border bg-white"
                >
                  <div className="flex items-center justify-between border-b border-zw-border p-3">
                    <div>
                      <div className="flex items-center gap-2">
                  ...
                  {order.status === "new" && (
                    <div className="flex gap-2 border-t border-zw-border p-3">
                      <button
                        onClick={() => updateOrderStatus(order.id, "accepted")}
                        className="flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl border border-zw-border bg-white text-[12px] font-semibold text-zw-text-secondary"
                      >
                        <X size={14} /> Decline
                      </button>
                      <button
                        onClick={() => updateOrderStatus(order.id, "accepted")}
                        className="flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl bg-zw-primary-700 text-[12px] font-semibold text-white"
                      >
                  ...
                  {order.status !== "new" && order.status !== "completed" && (
                    <div className="border-t border-zw-border p-3">
                      <button
                        ...
                        className="flex h-10 w-full items-center justify-center gap-1.5 rounded-xl bg-zw-primary-700 text-[12px] font-semibold text-white"
                      >
                        Mark as{" "}
                        {STATUS_LABELS[
                          STATUS_FLOW[
            ...
            >
              {shopListings.map((l, i) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${l.imageColor}`}
                    >
                    ...
                    <div className="flex flex-col gap-1">
                      <button className="flex h-7 w-7 items-center justify-center rounded-lg bg-zw-bg-muted text-zw-text-secondary">
                        <Edit size={12} />
                      </button>
                      <button className="flex h-7 w-7 items-center justify-center rounded-lg bg-zw-bg-muted text-zw-text-muted">
                        <Trash2 size={12} />
                      </button>
                    </div>
                  </motion.div>
...
function StatCard({
  ...
  return (
    <div className={`rounded-2xl border border-zw-border bg-gradient-to-br ${gradient} p-3.5`}>
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className="mt-1.5 font-display text-xl font-bold text-zw-text-primary"
      >
        {value}
      </motion.div>
      <div className="text-[10px] text-zw-text-muted">{trend}</div>
    </div>
  );
}
